...
import { EXTENDED_INDIAN_RECIPES } from '../data/extendedIndianRecipes';
 ...
 */
export function getAccurateFoodImage(title: string, cuisine?: string): string {
...
export async function fetchLargeRecipeDataset(): Promise<Recipe[]> {
  ...
  const addUniqueRecipe = (recipe: Recipe) => {
    ...
    if (!seenCanonicalKeys.has(canonicalKey)) {
      ...
      const accurateImage = getAccurateFoodImage(recipe.title, recipe.cuisine);
