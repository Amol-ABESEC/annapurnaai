...
function generateDynamicRecipe(dishQuery: string, servingsCount: number): string {
  ...
  const isSnack = /samosa|pakora|bhajji|tikki|chaat|panipuri|kebab|cutlet/i.test(name);
...
export function generateFallbackAssistantReply(
  ...
  // Helper to detect if query is a portion scaling or follow-up request
  const isPortionOrScalingQuery =
    lower.includes('for ') ||
