...
import { Calendar as CalendarIcon, ShoppingCart, Trash2, Clock, Flame, Sparkles, Plus, Utensils, Zap, Bot } from 'lucide-react';
import { MealPlanItem, DayOfWeek, MealSlot, Recipe, QuickCommPlatform, AutoPlanStrategy } from '../types';
import { generateSmartWeeklyMealPlan } from '../utils/AIMealPlanGenerator';
...
interface MealPlannerSectionProps {
  ...
  onAutoGeneratePlan?: (items: MealPlanItem[]) => void;
}
...
export const MealPlannerSection: React.FC<MealPlannerSectionProps> = ({
  ...
  const [autoStrategy, setAutoStrategy] = useState<AutoPlanStrategy>('zero_waste');
  const [autoDiet, setAutoDiet] = useState<'all' | 'vegetarian' | 'non-veg'>('all');
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-slate-950 font-extrabold text-xs shadow-lg shadow-amber-500/20 transition-all flex items-center gap-2 cursor-pointer"
            >
      ...
      {/* AI Auto-Plan Generator Modal */}
      {showAutoPlanModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#181311] border border-[#2a221f] rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-6 text-[#f0e6df] animate-in fade-in duration-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">AI Weekly Auto-Planner</h3>
                  <p className="text-xs text-[#a09088]">Generate a 7-day meal plan automatically</p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: 'zero_waste', label: '♻️ Zero Waste', desc: 'Max pantry match' },
                    { id: 'high_protein', label: '💪 High Protein', desc: 'Max protein g' },
                    { id: 'balanced', label: '⚖️ Balanced', desc: 'Top rated dishes' },
                  ].map((s) => (
                    <button
                      key={s.id}
                      type="button"
                      onClick={() => setAutoStrategy(s.id as AutoPlanStrategy)}
                      className={`p-3 rounded-2xl border text-left flex flex-col justify-between transition-all ${
                        autoStrategy === s.id
                          ? 'bg-amber-500/20 border-amber-500 text-amber-300 font-bold'
                          : 'bg-[#120f0e] border-[#2a221f] text-slate-400 hover:text-white'
                      }`}
                    >
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="button"
                onClick={() => setShowAutoPlanModal(false)}
                className="flex-1 py-3 bg-[#120f0e] hover:bg-[#241c19] text-slate-300 font-bold rounded-xl text-xs border border-[#2a221f]"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={() => {
                  const { generatedItems } = generateSmartWeeklyMealPlan(allRecipes, {
                    strategy: autoStrategy,
                    dietFilter: autoDiet,
                    targetDailyCalories: 2000,
                    targetDailyProtein: 80,
                  });
                  if (onAutoGeneratePlan) {
                    onAutoGeneratePlan(generatedItems);
                  }
                  setShowAutoPlanModal(false);
                }}
                className="flex-1 py-3 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-slate-950 font-extrabold rounded-xl text-xs shadow-lg shadow-amber-500/20"
              >
                Generate 7-Day Plan
              </button>
            </div>
