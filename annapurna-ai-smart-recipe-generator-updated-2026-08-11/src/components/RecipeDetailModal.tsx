...
import { X, Clock, Users, Flame, ChefHat, ShoppingCart, Sparkles, Play, Heart, Calendar, Trash2 } from 'lucide-react';
...
import { getSubstitutesForIngredient } from '../utils/substituteEngine';
...
export const RecipeDetailModal: React.FC<RecipeDetailModalProps> = ({
  ...
  const [selectedSpiceLevel, setSelectedSpiceLevel] = useState<'Mild' | 'Authentic' | 'Spicy'>('Authentic');
  const [activeSubstituteIngredient, setActiveSubstituteIngredient] = useState<string | null>(null);
  ...
  return (
    ...
    >
      <div className="bg-[#181311] border border-[#2a221f] rounded-t-3xl sm:rounded-3xl max-w-3xl w-full p-5 sm:p-6 shadow-2xl relative text-[#f0e6df] max-h-[92vh] sm:max-h-[88vh] overflow-y-auto animate-fadeIn">
        ...
        {/* Spice Level Intensity Adjuster */}
        <div className="mb-6 bg-[#241c19] border border-[#352a25] p-3.5 rounded-2xl flex items-center justify-between gap-3 text-xs">
          <span className="font-bold text-white flex items-center gap-1.5 font-serif">
            <Flame className="w-4 h-4 text-orange-500" />
            Spice Profile:
          </span>
          <div className="flex items-center gap-1.5">
            {(['Mild', 'Authentic', 'Spicy'] as const).map((lvl) => (
              <button
                key={lvl}
                type="button"
                ...
                className={`px-3 py-1.5 rounded-xl font-bold transition-all cursor-pointer border ${
                  selectedSpiceLevel === lvl
                    ? 'bg-[#ff6224] text-white border-[#ff6224] shadow-sm'
                    : 'bg-[#120f0e] text-slate-400 border-[#2a221f] hover:text-white'
                }`}
              >
                {lvl === 'Mild' ? '🌶️ Mild' : lvl === 'Authentic' ? '🌶️🌶️ Authentic' : '🌶️🌶️🌶️ Dhaba Spicy'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mb-6 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {scaledIngredients.map((ing, idx) => {
              ...
              const substitutes = ing.isMissing ? getSubstitutesForIngredient(ing.name, []) : [];
              ...
              return (
                ...
                >
                  ...
                  {/* Substitute Option Pill if Missing */}
                  {ing.isMissing && substitutes.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-red-500/20 flex flex-col gap-1">
                      <button
                        type="button"
                        onClick={() =>
                          setActiveSubstituteIngredient(
                            activeSubstituteIngredient === ing.name ? null : ing.name
                          )
                        }
                        className="text-[10px] text-amber-400 hover:text-amber-300 font-bold text-left underline underline-offset-2 flex items-center gap-1"
                      >
                        <span>🔄 Swap with substitute ({substitutes[0].substituteName})</span>
                      </button>

                      {activeSubstituteIngredient === ing.name && (
                        <div className="p-2 rounded-lg bg-[#120f0e] border border-amber-500/30 text-[10px] space-y-1 text-amber-200 mt-1">
                          <div><strong>Substitute:</strong> {substitutes[0].substituteName}</div>
                          <div><strong>Ratio:</strong> {substitutes[0].ratioText}</div>
                          <div className="text-slate-400">{substitutes[0].culinaryNotes}</div>
                        </div>
                      )}
                    </div>
                  )}
            ...
            <button
              onClick={() => {
                if (onOpenCookingMode) {
                  onOpenCookingMode(scaledRecipe);
                } else {
                  onOpenAIChatWithRecipe(recipe);
                }
              }}
              className="px-4 py-2 bg-gradient-to-r from-[#ff6224] to-amber-500 hover:from-[#e85418] hover:to-amber-600 text-white shadow-lg shadow-orange-500/20 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
            >
