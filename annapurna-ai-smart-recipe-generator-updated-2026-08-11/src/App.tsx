...
import { computeRecipesWithIndexedPantry, buildRecipeInvertedIndex } from './utils/recipeIndexer';
...
export default function App() {
  ...
  useEffect(() => {
    fetchLargeRecipeDataset().then((dataset) => {
      if (dataset && dataset.length > 0) {
        ...
        buildRecipeInvertedIndex(dataset);
      }
    });
  }, []);
  ...
  const computedRecipes: Recipe[] = useMemo(() => {
    ...
    return computeRecipesWithIndexedPantry(combinedRecipes, pantryItems);
  ...
  return (
    <div className="min-h-screen font-sans flex flex-col lg:flex-row relative bg-[#120f0e] text-[#f0e6df] selection:bg-[#ff6224]/30 selection:text-[#ff6224]">
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            {activeTab === 'planner' && (
              <MealPlannerSection
                ...
                onAutoGeneratePlan={(items) => setMealPlanItems(items)}
              />
            )}
      ...
      {/* Hands-Free Interactive Cooking Mode Modal */}
      {cookingModeRecipe && (
        <CookingModeModal
